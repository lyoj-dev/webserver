/**
 * @file file.cpp
 * @author LittleYang0531 (@domain.com)
 * @brief 断点续传文件示例
 * @version 1.0.2
 * @date 2022-09-01
 * 
...
int main(int argc, char** argv) {
    app.addRoute("/sample.mp4", [](client_conn conn, http_request request, param argv) {
        /** 打开文件 */
        ifstream fin("video.mp4", ios::binary);
        if (!fin.is_open()) {
        ...
        response = merge(response, mime(".mp4"));
